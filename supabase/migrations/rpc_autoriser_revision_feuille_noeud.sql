-- ================================================================
-- RPC : autoriser_revision_feuille — mise à jour pour table noeud
--
-- Changements par rapport à autoriser_revision_feuille.sql :
--   FROM feuille_entrainement WHERE id = p_feuille_id
--   → FROM noeud WHERE id = p_feuille_id
--
--   JOIN feuille_entrainement fe ON fa.feuille_id = fe.id
--   → JOIN noeud fe ON fa.feuille_id = fe.id
--
...
BEGIN
    ...
    -- Lire le type depuis noeud (au lieu de feuille_entrainement)
    ...
    WHERE fa.membre_id = p_membre_id
      ...
      AND fa.feuille_id != p_feuille_id
