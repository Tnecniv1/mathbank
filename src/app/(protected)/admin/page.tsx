...
function Button({
 ...
 const variantClass = {
 primary: 'bg-accent hover:bg-teal-700 text-ink shadow-sm hover:shadow-sm',
 secondary: 'bg-cream-200 hover:bg-cream-100 text-slate-800',
 danger: 'bg-red-600 hover:bg-red-700 text-ink shadow-sm',
 ghost: 'hover:bg-cream-200 text-ink-light',
 }[variant];
...
function Input({
 ...
 required={required}
 className="w-full px-3 py-2 rounded-lg border border-border bg-cream-50 text-slate-800 focus:outline-none focus:ring-2 focus:ring-teal-500"
 />
 </div>
 );
}
...
function Textarea({
 ...
 return (
 <div>
 <label className="block text-sm font-medium mb-1.5 text-ink">{label}</label>
 <textarea
 value={value}
 onChange={(e) => onChange(e.target.value)}
 placeholder={placeholder}
 rows={rows}
 className="w-full px-3 py-2 rounded-lg border border-border bg-cream-50 text-slate-800 focus:outline-none focus:ring-2 focus:ring-teal-500 resize-none"
 />
 </div>
 );
}
...
function Modal({
 ...
 <div
 className="bg-cream-50 rounded-lg shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto"
 onClick={(e) => e.stopPropagation()}
 >
 <div className="sticky top-0 bg-cream-50 border-b border-border px-6 py-4 flex items-center justify-between">
 <h2 className="text-xl font-bold text-slate-800">{title}</h2>
 <button onClick={onClose} className="p-1 hover:bg-cream-200 rounded transition-colors">
 <IconCancel />
 </button>
 </div>
...
function PdfUpload({ onFileSelect, selectedFile, onRemove, uploading = false }: {
 ...
 if (selectedFile) {
 return (
 <div className="p-4 rounded-lg border border-teal-500 bg-teal-50">
 ...
 return (
 <div
 className={`relative p-8 rounded-lg border border-dashed transition-all cursor-pointer ${
 dragActive
 ? 'border-teal-500 bg-teal-50'
 : 'border-border hover:border-teal-400 hover:bg-cream-100'
 }`}
 ...
 <p className="text-sm text-ink-muted">PDF uniquement, max 10 MB</p>
 </div>
 </div>
 );
}
...
function ItemActions({
 ...
 onClick={onMoveUp}
 disabled={!canMoveUp}
 className="p-1.5 hover:bg-cream-200 rounded transition-colors disabled:opacity-30 disabled:cursor-not-allowed"
 title="Monter"
 >
 <IconArrowUp />
 </button>
 ...
 <button
 onClick={onEdit}
 className="p-1.5 hover:bg-cream-200 rounded transition-colors"
 title="Modifier"
 >
 <IconEdit />
 </button>
...
function FeuilleItem({
 ...
 ? 'bg-accent-light border-blue-300 text-accent'
...
function ChapitreItem({
 ...
 <div
 className="flex items-center justify-between p-3 from-pink-100 to-pink-50 cursor-pointer transition-all"
 onClick={onToggle}
 >
 <div className="flex items-center gap-3 flex-1 min-w-0">
 <IconChevron open={isOpen} />
 <div className="flex-1 min-w-0">
 <div className="font-semibold text-slate-800 truncate">{chapitre.titre}</div>
 ...
 <div className="p-4 space-y-2 bg-cream-50">
 ...
 {feuilles.length === 0 ? (
 <p className="text-center text-ink-muted py-4">Aucune feuille</p>
 ) : (
...
function SujetItem({
 ...
 <div
 className="flex items-center justify-between p-3 cursor-pointer hover:hover: transition-all"
 onClick={onToggle}
 >
 <div className="flex items-center gap-3 flex-1 min-w-0">
 <IconChevron open={isOpen} />
 <div className="flex-1 min-w-0">
 <div className="font-bold text-slate-800 truncate">{sujet.titre}</div>
 <div className="text-xs text-ink-light">
...
function NiveauItem({
 ...
 return (
 <div className="border border-teal-300 rounded-lg overflow-hidden shadow-sm">
 <div
 className="flex items-center justify-between p-4 cursor-pointer hover:hover: transition-all"
 onClick={onToggle}
 >
 <div className="flex items-center gap-3 flex-1 min-w-0">
 <IconChevron open={isOpen} />
 <div className="flex-1 min-w-0">
 <div className="font-extrabold text-lg text-slate-800 truncate">{niveau.titre}</div>
 <div className="text-xs text-ink-light">
 ...
 <Collapse open={isOpen}>
 <div className="p-5 space-y-4 bg-cream-50">
 <div className="flex justify-end mb-2">
 ...
 {sujets.length === 0 ? (
 <p className="text-center text-ink-muted py-6">Aucun sujet</p>
 ) : (
 ...
 *,
 ...
 <Loader />
 <span className="text-lg text-slate-800">Chargement...</span>
 </div>
 </div>
 );
 }
 ...
 <span className="text-slate-800">Administration</span>
 </h1>
 <p className="text-ink-muted">Gérez la bibliothèque et les demandes d'équipes</p>
 </div>
 </div>
 ...
 👥 Demandes d'équipes
 {demandes.length > 0 && (
 <span className="absolute -top-2 -right-2 bg-red-500 text-ink text-xs font-bold rounded-full w-6 h-6 flex items-center justify-center animate-pulse">
 {demandes.length}
 </span>
 )}
 </button>
 ...
 {/* Contenu */}
 {niveaux.length === 0 ? (
 <div className="text-center py-16 bg-cream-50 rounded-lg border border-dashed border-border">
 <p className="text-ink-muted mb-4">Aucun niveau créé</p>
 <Button onClick={() => openModal('niveau')}>
 <IconPlus />
 ...
 {demandes.length > 0 && (
 <span className="px-4 py-2 bg-orange-100 text-orange-700 font-semibold rounded-full">
 ...
 <Loader />
 <p className="mt-4 text-ink-light">Chargement...</p>
 </div>
 ) : demandes.length === 0 ? (
 ...
 <div key={demande.id} className="p-6 bg-cream-100 rounded-lg border border-border">
 ...
 {demande.description && (
 <p className="mt-3 p-3 bg-cream-50 rounded-lg border">"{demande.description}"
 </p>
 )}
 </div>
 </div>
 ...
 className="flex-1 px-5 py-3 bg-green-500 hover:bg-green-600 text-ink font-semibold rounded-lg"
 >
 ✓ Approuver
 </button>
 <button
 onClick={() => handleRefuserDemande(demande.id)}
 className="flex-1 px-5 py-3 bg-red-500 hover:bg-red-600 text-ink font-semibold rounded-lg"
 >
 ✗ Refuser
 </button>
 </div>
