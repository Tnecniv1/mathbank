...
export default function AssistePage() {
  ...
  // ── Lire ?entrainement_id depuis l'URL ──────────────────────────────────
  useEffect(() => {
  ...
  // ── Créer ou récupérer l'entraînement ──────────────────────────────────
  const ensureEntrainement = useCallback(async (): Promise<string | null> => {
    ...
    const { data, error } = await supabase
      .from('entrainement')
      .insert({ user_id: session.user.id, statut: 'en_cours', mode: 'assiste' })
      .select('id')
      .single();

    if (error || !data) {
      console.error('[assiste] create entrainement error:', error);
      return null;
    }

    setEntrainementId(data.id);
    return data.id;
  }, [entrainementId]);

  // ── Boucler l'entraînement ─────────────────────────────────────────────
  const bouclerEntrainement = useCallback(async () => {
    if (!entrainementId) return;
    await supabase
      .from('entrainement')
