...
export default function SessionPage() {
  ...
  // Roue en cours (provenant du prof)
  ...
  const sendMessage = useCallback(
    ...
    [feuille, input, messages, pendingImage, prenom, currentExercice, roueEnCoursActif]
  );

  // ── Démarrage de la session ─────────────────────────────────────────────
  const startSession = useCallback(async (exerciceOverride?: number, roueEnCours?: RoueEnCours) => {
    if (!feuille) return;
    ...
    console.log('[startSession] exo =', exo, 'roueEnCours =', roueEnCours ?? null);
    try {
  ...
  }, [feuille, lastPhotoSent, messages, currentExercice, roueEnCoursActif, sessionStartTime]);

  const handleNextExercice = useCallback(() => {
    startSession(currentExercice + 1);
  }, [startSession, currentExercice]);

  const handleContinuerExercice = useCallback(() => {
    if (!feuille) return;
    startSession(currentExercice, roueEnCoursActif ?? undefined);
  }, [feuille, startSession, currentExercice, roueEnCoursActif]);

  const handleTerminerSession = useCallback(async () => {
    if (roueEnCoursActif && feuille) {
      // Fermer la roue sans réévaluer
      await fetch('/api/session/evaluate', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          feuille_id: feuille.id,
          roue_id: roueEnCoursActif.roue_id,
          boucler: true,
          messages: [],
          exercice_numero: currentExercice,
          type: feuille.type,
          reference: roueEnCoursActif.reference,
          photo_base64: '',
          photo_mime: 'image/jpeg',
          session_start_time: sessionStartTime ?? Date.now(),
        }),
      }).catch((err) => console.error('[terminer session] close roue error:', err));
    }
    setStarted(false);
    setMessages([]);
    setEvaluation(null);
    setLastPhotoSent(null);
    setRoueEnCoursActif(null);
    setCurrentExercice(feuille?.prochain_exercice ?? 1);
  }, [roueEnCoursActif, feuille, currentExercice, sessionStartTime]);
      ...
      {!started && (
        <div className="flex-1 overflow-y-auto">
          <div className="flex flex-col items-center px-4 py-6 gap-6 max-w-lg mx-auto">
            ...
            {/* Card roue en cours (observée en classe par le prof) */}
            {!sessionEnCours && feuille?.roue_en_cours && (
              <div className="bg-[#F0F4FF] border border-[#B0C4DE] rounded-2xl p-6 w-full space-y-4 shadow-sm">
                <div className="space-y-1">
                  ...
                  <p className="text-sm text-[#555]">
                    Commencé en classe — critères partiellement observés
                  </p>
                </div>
            ...
            <div className="bg-white border-t border-[#E8E8E8] px-4 py-4 shrink-0">
              <div className="max-w-2xl mx-auto space-y-3">
                ...
                <div className="flex flex-col gap-2">
                  {roueEnCoursActif && (
                    <button
                      onClick={handleContinuerExercice}
                      className="w-full py-2.5 rounded-xl bg-white border border-[#2D3A8C] text-[#2D3A8C]
                                 font-semibold text-sm hover:bg-[#F0F4FF] transition-colors"
                    >
