...
export default function GestionPrerequisAdmin() {
        ...
        data: {
          label: (
            <div className="text-center">
              <div className="font-bold text-blue-600">Feuille sélectionnée</div>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white dark:bg-slate-900 rounded-xl p-6 border-2 border-slate-300 dark:border-slate-700">
        ...
        {feuilleSelectionnee && (
          <div className="mb-4 p-3 bg-blue-50 dark:bg-blue-900/20 border-2 border-blue-300 dark:border-blue-700 rounded-lg">
            ...
            <button
              ...
              className="mt-2 px-3 py-1 bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium rounded"
            >
              Changer de feuille
            </button>
          </div>
        ...
        {!feuilleSelectionnee && (
          <div className="space-y-2">
            {Object.entries(hierarchie).map(([niveau, sujets]) => (
              <div key={niveau} className="border-2 border-slate-200 dark:border-slate-700 rounded-lg overflow-hidden">
                <button
                  onClick={() => toggleNiveau(niveau)}
                  className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-800 hover:bg-slate-100 dark:hover:bg-slate-700 flex items-center justify-between text-left transition-colors"
                >
                ...
                {openNiveaux.has(niveau) && (
                  <div className="p-2 space-y-2 bg-white dark:bg-slate-900">
                    {Object.entries(sujets as any).map(([sujet, chapitres]) => (
                      <div key={sujet} className="border border-slate-200 dark:border-slate-700 rounded-lg overflow-hidden">
                        <button
                          onClick={() => toggleSujet(`${niveau}-${sujet}`)}
                          className="w-full px-3 py-2 bg-slate-50 dark:bg-slate-800 hover:bg-slate-100 dark:hover:bg-slate-700 flex items-center justify-between text-left text-sm transition-colors"
                        >
                        ...
                        {openSujets.has(`${niveau}-${sujet}`) && (
                          <div className="p-2 space-y-1 bg-white dark:bg-slate-900">
                            {Object.entries(chapitres as any).map(([chapitre, feuillesList]) => (
                              <div key={chapitre} className="border border-slate-100 dark:border-slate-800 rounded overflow-hidden">
                                <button
                                  ...
                                  <div className="p-2 space-y-1 bg-white dark:bg-slate-900">
                                    {(feuillesList as Feuille[]).map(feuille => (
                                      <button
                                        ...
                                        className="w-full text-left px-3 py-2 rounded text-sm bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 hover:bg-blue-50 dark:hover:bg-blue-900/20 hover:border-blue-300 dark:hover:border-blue-700 transition-colors"
                                      >
        ...
        <>
                ...
                {prerequisActuels.map(p => (
                  ...
                  >
                    ...
                    <button
                      ...
                      disabled={saving}
                      className="px-3 py-1.5 bg-red-500 hover:bg-red-600 disabled:bg-slate-300 text-white text-sm font-medium rounded-lg transition-colors"
                    >
                      Retirer
                    </button>
                  </div>
          ...
          <div className="bg-white dark:bg-slate-900 rounded-xl p-6 border-2 border-slate-300 dark:border-slate-700">
            ...
            <div className="space-y-4">
              <div>
                ...
                {nouveauPrerequisId ? (
                  <div className="mb-3 p-3 bg-green-50 dark:bg-green-900/20 border-2 border-green-300 dark:border-green-700 rounded-lg">
                    <div className="text-sm font-medium text-green-900 dark:text-green-300">
                      ✅ Prérequis sélectionné :
                    </div>
                    <div className="font-bold text-green-800 dark:text-green-200 mt-1">
                    ...
                    <button
                      ...
                      className="mt-2 px-3 py-1 bg-green-600 hover:bg-green-700 text-white text-sm font-medium rounded"
                    >
                      Changer de feuille
                    </button>
                  </div>
                ) : (
                  <div className="space-y-2 max-h-96 overflow-y-auto border-2 border-slate-300 dark:border-slate-700 rounded-lg p-2">
                    {Object.entries(hierarchie).map(([niveau, sujets]) => (
                      <div key={niveau} className="border-2 border-slate-200 dark:border-slate-700 rounded-lg overflow-hidden">
                        <button
                          onClick={() => toggleNiveau(niveau)}
                                            ...
                                            {(feuillesList as Feuille[])
                                              ...
                                              .map(feuille => (
                                                <button
                                                  ...
                                                  className="w-full text-left px-3 py-2 rounded text-sm bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 hover:bg-green-50 dark:hover:bg-green-900/20 hover:border-green-300 dark:hover:border-green-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
                                                >
                                                  <span className="text-slate-900 dark:text-white">
                                                    #{feuille.ordre} - {feuille.titre}
                                                  </span>
                                                  {prerequisActuels.some(p => p.prerequis_id === feuille.id) && (
                                                    <span className="ml-2 text-xs text-slate-500">(déjà ajouté)</span>
                                                  )}
                                                </button>
                                              ))}
                                          </div>
              ...
              <div className="flex items-center gap-2">
                <input
                  ...
                  disabled={saving}
                />
                <label htmlFor="obligatoire" className="text-sm font-medium text-slate-700 dark:text-slate-300">
              ...
              <button
                ...
                className="w-full px-4 py-3 bg-teal-500 hover:bg-teal-600 disabled:bg-slate-300 text-white font-semibold rounded-lg transition-colors"
              >
          ...
          {prerequisActuels.length > 0 && (
            <div className="bg-white dark:bg-slate-900 rounded-xl p-6 border-2 border-slate-300 dark:border-slate-700">
              ...
              <div style={{ height: '400px', width: '100%' }} className="border-2 border-slate-300 dark:border-slate-700 rounded-lg overflow-hidden">
                <ReactFlow
                  nodes={nodes}
                  edges={edges}
